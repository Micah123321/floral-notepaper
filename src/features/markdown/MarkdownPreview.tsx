...
const sanitizeSchema = {
  ...
  protocols: {
    ...
    href: [...(defaultSchema.protocols?.href ?? []), "floral-attachment"],
    src: [...(defaultSchema.protocols?.src ?? []), "floral-attachment"],
  },
};
...
export function resolveAttachmentReference(
  ...
  if (!href?.startsWith("floral-attachment://")) {
    return null;
  }

  return attachments.find((attachment) => attachment.markdownUrl === href) ?? null;
}

function makeComponents(attachments: NoteAttachment[]): Components {
  return {
    h1: ({ children }) => (
    ...
    a: ({ href, children }) => {
      const attachment = resolveAttachmentReference(href, attachments);
      return (
        <a
          href={href}
          onClick={(e) => {
    ...
    img: ({ src, alt }) => {
      const attachment = resolveAttachmentReference(src, attachments);
      const imageSrc = attachment ? convertFileSrc(attachment.path) : src;
      return (
        <img
          src={imageSrc}
          alt={alt ?? ""}
          className="my-4 max-h-[420px] max-w-full rounded-lg border border-paper-deep/60 bg-paper-warm/40 object-contain shadow-[0_10px_28px_rgba(26,26,24,0.08)]"
          loading="lazy"
        />
      );
    },
...
export function MarkdownPreview({
  ...
  const { t } = useTranslation();
  const components = makeComponents(attachments);
  return (
